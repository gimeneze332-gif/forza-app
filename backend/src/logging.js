const ALLOWED = new Set(["requestId", "timestamp", "status", "durationMs", "size", "componentCount", "modelCount", "quotaRemaining", "error", "provider", "model", "inputTokens", "outputTokens", "thinkingTokens", "totalTokens", "neurons"]);
const DIAGNOSTIC_FIELDS = new Set(["requestId", "stage", "provider", "model", "durationMs", "size", "status", "providerHttpStatus", "providerErrorStatus", "usageAvailable", "inputTokens", "outputTokens", "thinkingTokens", "totalTokens", "neurons", "errorCode", "canonicalizationErrorCode", "selectedWrapper", "candidateType", "rootKeyProfile", "unknownRootKeysCount", "resultShape", "dataShape", "outputShape", "rootShapeCodes"]);
const DIAGNOSTIC_STAGES = new Set(["request_received", "model_call_started", "model_call_completed", "model_call_failed", "query_response_rejected", "empty_response", "json_extraction_failed", "json_parse_failed", "schema_validation_failed", "no_food", "low_confidence", "normalization_completed", "analysis_completed", "gemini_request_started", "gemini_response_received", "gemini_auth_failed", "gemini_billing_failed", "gemini_rate_limited", "gemini_model_not_found", "gemini_request_invalid", "gemini_provider_unavailable", "gemini_timeout", "gemini_response_empty", "gemini_json_parse_failed", "gemini_schema_failed", "gemini_usage_received", "gemini_completed"]);
...
export function safeStageLog(details = {}, output = console) {
  ...
  for (const [key, value] of Object.entries(details)) {
    ...
    if (["durationMs", "size", "status", "providerHttpStatus", "inputTokens", "outputTokens", "thinkingTokens", "totalTokens", "neurons", "unknownRootKeysCount"].includes(key) && !Number.isFinite(Number(value))) continue;
    safe[key] = ["durationMs", "size", "status", "providerHttpStatus", "inputTokens", "outputTokens", "thinkingTokens", "totalTokens", "neurons", "unknownRootKeysCount"].includes(key) ? Number(value) : value;
  }
  output.log(JSON.stringify(safe));
  return safe;
}
