...
(async () => {
  ...
  const stageLogs = [];
  const safeStage = safeStageLog({ requestId: "id", stage: "gemini_request_invalid", provider: "gemini", model: "gemini-3.5-flash-lite", providerHttpStatus: 400, providerErrorStatus: "INVALID_ARGUMENT", errorCode: "provider_request_failed", usageAvailable: false, prompt: PHOTO_FOOD_PROMPT, response: valid, image: "base64", apiKey: "secret", foods: ["pollo"], message: "mensaje sensible" }, { log: line => stageLogs.push(line) });
  assert.deepEqual(safeStage, { requestId: "id", stage: "gemini_request_invalid", provider: "gemini", model: "gemini-3.5-flash-lite", providerHttpStatus: 400, providerErrorStatus: "INVALID_ARGUMENT", errorCode: "provider_request_failed", usageAvailable: false });
