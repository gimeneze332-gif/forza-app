...
(async () => {
  ...
  assert.deepEqual(result.usage, { inputTokens: 300, outputTokens: 80, thinkingTokens: 0, totalTokens: 380, inputCostUsd: 0.00009, outputCostUsd: 0.0002, estimatedCostUsd: 0.00029 });
  ...
  assert.equal(diagnostics.every(item => item.model === "gemini-3.5-flash-lite"), true, "stages Gemini registran modelo exacto");
  ...
  assert.equal(diagnostics[2].inputCostUsd, 0.00009);
  assert.equal(diagnostics[2].outputCostUsd, 0.0002);
  assert.equal(diagnostics[2].estimatedCostUsd, 0.00029);
  ...
  let retryCount = 0;
  await assert.rejects(analyzeFoodImage(new Uint8Array([1]), { apiKey: "x", fetchImpl: async () => { retryCount += 1; return response({ error: { status: "INTERNAL" } }, 500); } }), /provider_unavailable/, "error Gemini controlado");
  assert.equal(retryCount, 1, "Gemini no se reintenta automáticamente");
  ...
  const logs = []; const safe = safeLog({ requestId: "id", provider: "gemini", inputTokens: 300, outputTokens: 80, thinkingTokens: 10, totalTokens: 390, inputCostUsd: 0.00009, outputCostUsd: 0.000225, estimatedCostUsd: 0.000315, prompt: PHOTO_FOOD_PROMPT, response: valid, image: "base64", apiKey: "secret", foods: ["pollo"] }, { log: line => logs.push(line) });
  assert.deepEqual(Object.keys(safe).sort(), ["estimatedCostUsd", "inputCostUsd", "inputTokens", "outputCostUsd", "outputTokens", "provider", "requestId", "thinkingTokens", "totalTokens"]);
  assert.equal(/secret|pollo|base64|observador visual/.test(logs[0]), false, "logs sin contenido sensible");
  const stageLogs = [];
  const safeStage = safeStageLog({ requestId: "id", stage: "gemini_usage_received", provider: "gemini", model: "gemini-3.5-flash-lite", providerHttpStatus: 200, usageAvailable: true, inputTokens: 300, outputTokens: 80, thinkingTokens: 10, totalTokens: 390, inputCostUsd: 0.00009, outputCostUsd: 0.000225, estimatedCostUsd: 0.000315, prompt: PHOTO_FOOD_PROMPT, response: valid, image: "base64", apiKey: "secret", foods: ["pollo"], message: "mensaje sensible" }, { log: line => stageLogs.push(line) });
  assert.deepEqual(safeStage, { requestId: "id", stage: "gemini_usage_received", provider: "gemini", model: "gemini-3.5-flash-lite", providerHttpStatus: 200, usageAvailable: true, inputTokens: 300, outputTokens: 80, thinkingTokens: 10, totalTokens: 390, inputCostUsd: 0.00009, outputCostUsd: 0.000225, estimatedCostUsd: 0.000315 });
